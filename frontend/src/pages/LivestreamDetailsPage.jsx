...
import { useAuth } from '../contexts/AuthContext'
import { Badge, Button, Card, CardBody, LoadingScreen, SectionHeader } from '../components/UI'
import { fetchLivestream, joinQueue, leaveQueue } from '../services/livestreams'
import { useLivestreamSocket } from '../hooks/useLivestreamSocket'
import { formatDateTime, formatStatus, statusTone } from '../utils/format'
...
export default function LivestreamDetailsPage() {
  ...
  if (!details) {
    return <div className="text-muted">Không tìm thấy livestream.</div>
  }

  const myEntry = queue.find(
    (entry) =>
      entry.user_id === user?.id &&
      ['waiting', 'playing'].includes(entry.status)
  )
      ...
      <SectionHeader
        ...
        actions={
          <>
            ...
            {user && details.status === 'open' ? (
              myEntry ? (
                <Button tone="secondary" onClick={handleLeave}>
                  Rời hàng chờ
                </Button>
              ) : (
            ...
            {details.streamer_id === user?.id || user?.role === 'admin' ? (
              <Link to={`/streamer/livestreams/${details.id}/manage`}>
        ...
        <Card className="xl:col-span-2">
          <CardBody className="space-y-4">
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                ...
                >
                  <div>
                    <div className="font-medium text-text">
                      {entry.user?.ingame_name}
                    </div>

                    <div className="text-xs text-muted">
                      {entry.user?.game_uid || 'Chưa có hồ sơ trong game'}
                    </div>
                  </div>
              ...
              {!queue.length ? (
                <div className="rounded-xl border border-white/8 bg-white/4 px-4 py-6 text-sm text-muted">
                  Hiện chưa có ai trong hàng chờ.
                </div>
              ) : null}
              ...
              {queue.length > 20 ? (
                <div className="pt-2 text-center">
                  ...
                  >
                    {showAllQueue
                      ? 'Thu gọn hàng chờ'
                      : `Xem thêm ${queue.length - 20} người`}
                  </Button>
                </div>
              ) : null}
          ...
          <Card>
            <CardBody className="space-y-3">
              ...
              {playing.map((entry) => (
                <div
                  key={entry.id}
                  className="rounded-xl bg-white/5 px-4 py-3 text-sm text-text"
                >
                  {entry.user?.ingame_name}
                </div>
              ))}
          ...
          <Card>
            <CardBody className="space-y-2 text-sm text-muted">
              ...
              <div className="flex justify-between">
                <span>Số suất mỗi trận</span>
                <span className="text-text">
                  {details.slot_per_match}
                </span>
              </div>
